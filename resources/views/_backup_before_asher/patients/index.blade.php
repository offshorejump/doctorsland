  ...
  <div class="box-body">
    ...
    <table id="patientdatatable" class="table table-bordered table-striped">
      ...
      <tr>
        <td>{{$patient->name}}</td>
        <td>{{$patient->email}}</td>
        <td> @if( !empty( $patient->email ) ) <i class="fa fa-envelope-o" aria-hidden="true"></i> {{$patient->email}} @endif
        	<br />
        	@if( !empty( $patient->phone ) ) <a data-original-title="Call" href="tel:{{$patient->phone}}"> <i class="fa fa-envelope-o" aria-hidden="true"></i> {{$patient->phone}}</a> @endif
         </td>
