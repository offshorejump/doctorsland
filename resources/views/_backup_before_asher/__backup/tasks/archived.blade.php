...
@section('title') Archived Tasks @stop

@section('breadcrumbs', '<li class="active">Archived Tasks</li>')

@section('content')

    <div class="box">
        <div class="box-header">
            <h3 class="box-title">Archived Tasks</h3>
        </div>
        <!-- Admins-header -->
        <div class="box-body">
                        ...
                        <div class="notes-title-list">
                            ...
                            <ul class="nav nav-tabs" role="tablist">
                                ...
                                @foreach($task_list as $items)
                                    <li><a href="{{url('archived_tasks',['id' => $items->id])}}">{{$items->title}}</a></li>
                                @endforeach
                        ...
                        @foreach($task_id as $item)
                            <div class="task-right">
                                <div class="alert alert-warning">
                                    <a href="javascript:;" data-status="{{$item->status}}"  class="close" data-toggle="tooltip" id="archive_btn" title="Remove from Archive"><i class="fa fa-archive"></i> <button class="task_id hidden" value="{{$item->id}}" class="hidden">{{$item->id}}</button> </a>
                                    Archived task.
                                </div>
    ...
    <script>
        ...
        $("#archive_btn").on('click',function(){
            ...
            $.ajax({
                ...
                success:function(data){
                    window.location.href = "{{ url('/archived_tasks')}}";
                },
                error:function(){
                    alert('failed');
                }
